...
async function deletePdf(item) {
  try {
    ...
    await supabase.storage.from('pdfs').remove([pdfFileName, thumbFileName]);
...
zoomIn.onclick = () => { currentScale += 0.2; renderPage(); };
zoomOut.onclick = () => { if (currentScale > 0.5) { currentScale -= 0.2; renderPage(); } };
